...
# ============ ADDED: import your new pipeline stages ============
...
app = Flask(__name__)
CORS(app)
app.json.ensure_ascii = False   # Flask 3.x: this is the only line that works
...
def transcribe():
    ...
    try:
        ...
        # ============ ADDED: padya matching + story generation ============
        # Only run the pipeline if we actually got usable text back from ASR.
        if cleaned_text.strip():
            match_result = find_best_match(cleaned_text)
            story = generate_story(match_result["summary"])
            matched_padya = match_result["matched_padya"]
            match_distance = match_result["distance"]
        else:
            story = "ಆಡಿಯೊದಿಂದ ಪಠ್ಯ ಗುರುತಿಸಲು ಸಾಧ್ಯವಾಗಲಿಲ್ಲ."  # "Could not recognize text from audio."
            matched_padya = None
            match_distance = None
        # =====================================================================
        ...
        return jsonify({
            ...
            "cleaned_text": cleaned_text,
            "formatted_padya": display_padya,
            "matched_padya": matched_padya,     # ADDED: which dataset row was matched
            "match_distance": match_distance,   # ADDED: useful for debugging match quality
            "story": story                      # CHANGED: was a hardcoded placeholder string
        })
